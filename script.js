...
function drawMenu()
{
    ...
    return {minFretInput: minFretInput, 
...
function startMenu()
{
    ...
    menuInputs.goButton.onclick = () =>
    {
        ...
        let inputs = {  minFret: parseInt(menuInputs.minFretInput.value),
...
async function launch(inputs)
{
    ...
    counter.innerHTML = 1;
    await delay(1000);
    startGame(inputs.minFret, inputs.maxFret, inputs.time, inputs.baseNote);
}

async function startGame(minFret, maxFret, time, baseNote)
{
    //begin actual game
    ...
    //create an array with each fret in selected range in ascending order contained in it, then shuffle it
    let frets = [];
    ...
    for(let i = 0; i < frets.length; i++)
    {
        ...
        //THE FOLLOWING IS AN EMBARASSING BODGE JOB FIX FOR AN ISSUE 
        if(i == frets.length - 1)
        {
            done = true;
        }
